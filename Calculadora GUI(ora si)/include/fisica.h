        ...
        case 6: //conversiones
            ...
            case 0:
                ...
                if (numero > 1) {//conversion de arriba hacia abajo
                    ...
                    numero *= 10;
                    exponente2--;
                    sprintf(res, "%.20f*10^%d", numero, exponente2);
                    a = res;
                    return a;
                }
...
long double TiroVertical(long double altura, long double tiempo, long double vel_inicial, int seleccion){
    switch(seleccion){
        case 0:
            ...
            if(vel_inicial != -1048576){
                if(tiempo != -1048576){
                    altura = vel_inicial / 2 * tiempo;
                }
                else{
                    altura = pow(vel_inicial, 2) / (2 * -gravedad) * -2;
                }
            }
            else{
                altura = vel_inicial * tiempo + (pow(tiempo * -gravedad, 2) / 2);
            }
            return altura;
        case 1:
            if(tiempo != -1048576){
