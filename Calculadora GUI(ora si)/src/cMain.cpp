...
cMain::cMain() : wxFrame(nullptr, wxID_ANY, "Calculadora", wxPoint(10, 10), wxSize(625, 575)){
    ...
    wxFont Pequenia(12, wxFONTFAMILY_MODERN, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_EXTRABOLD);
    SetFont(Num);
    ...
    m_tree1 = new wxTreeCtrl(this, 10050, wxPoint(320, 90), wxSize(280, 435), wxTR_HAS_BUTTONS);
    //Arbol:
    ...
    if(strlen(numeros) != 0){
      ...
      if(nArgumentos != 0){
        ...
        if(Argumentos_current == nArgumentos){ //todos los argumentos han sido recibidos
          /*
          *la operacion requeria solo un argumento
          */
           ...
           */
            ...
            notacion_usado = true;
            m_txt2 -> SetValue(Notacion(Argumentos[0], Argumentos[1], INDEF, INDEF, 6, 0));
          }
